...
for epoch in range(epoch_num):
    ...
    if loss_tmp < loss_past:
        loss_past = loss_tmp
        PATH = f"Classifier_all_samples\\VGG11-all-samples.pt"
...
results_df = []
cls_model = torch.load(f"Classifier_all_samples\\VGG11-all-samples.pt")
cls_model.eval()
prob_list = []
label_list = []
threshold = 0.5
...
results_df.to_csv("Classifier_performance\\VGG11-all-samples.csv", index=False, encoding="cp1252")

...
# Model initialization using all samples
train_csv_tmp = pd.read_csv(f"covid_train_50.csv")
...
for epoch in range(epoch_num):
    ...
    with torch.no_grad():
        for data in tqdm(train_loader_cls_loss):
            ...
            prob_list = np.concatenate((prob_list, torch.sigmoid(outputs)[:, 1].detach().cpu().numpy()),
                                       axis=None)
    prob_list = np.clip(prob_list, 1e-8, 1 - 1e-8)
    ...
    if loss_tmp < loss_past:
        loss_past = loss_tmp
        PATH = f"Classifier_all_samples\\txrv-all-samples.pt"
...
results_df = []
cls_model = torch.load(f"Classifier_all_samples\\txrv-all-samples.pt")
cls_model.eval()
prob_list = []
label_list = []
threshold = 0.5
...
results_df.append([f"All samples",
...
results_df.to_csv("Classifier_performance\\MLP3-all-samples-txrv.csv", index=False, encoding="cp1252")

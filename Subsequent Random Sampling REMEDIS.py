...
for sample_id in range(100):
    for sub_bug in range(10, 41, 10):
        if sub_bug == 10:
            ...
            if os.path.exists(f"Classifier_random\\remedis-random-{sample_id}-10.pt"):
                cls_model_pre = torch.load(f"Classifier_random\\remedis-random-{sample_id}-10.pt")
                ...
                train_csv_now.to_csv(f"Sample_random\\remedis_train_{sample_id}_{bug}_10.csv", index=False)
            else:
                train_csv_now = pd.read_csv(f"Sample_random\\train_{sample_id}_20.csv")
                train_csv_now.to_csv(f"Sample_random\\remedis_train_{sample_id}_{bug}_10.csv", index=False)
        else:
            train_csv_pre = pd.read_csv(f"Sample_random\\remedis_train_{sample_id}_{bug}_{sub_bug-10}.csv")
            if os.path.exists(f"Classifier_random\\remedis-random-{sample_id}-{bug}-{sub_bug-10}.pt"):
                cls_model_pre = torch.load(f"Classifier_random\\remedis-random-{sample_id}-{bug}-{sub_bug-10}.pt")
                train_csv = pd.read_csv(f"covid_train_50.csv")
                train_csv_available = pd.concat([train_csv_pre, train_csv]).drop_duplicates(keep=False)
                train_loader_available = torch.utils.data.DataLoader(
                    Lung_cls_embed(csv_file=train_csv_available, embed_file=embed_file), batch_size=1, shuffle=False)
                ...
                train_csv_now = pd.concat([train_csv_pre, train_csv_available.iloc[np.concatenate([id_array])]]). \
                    drop_duplicates().reset_index(drop=True)
                train_csv_now.to_csv(f"Sample_random\\remedis_train_{sample_id}_{bug}_{sub_bug}.csv", index=False)
            else:
        ...
        for epoch in range(epoch_num):
            ...
            if loss_tmp < loss_past:
                loss_past = loss_tmp
                PATH = f"Classifier_random\\remedis-random-{sample_id}-{bug}-{sub_bug}.pt"
...
for sub_bug in range(10, 41, 10):
    ...
    bootstrap_npv = []
    for sample_id in range(100):
        if not os.path.exists(f"Classifier_random\\remedis-random-{sample_id}-{bug}-{sub_bug}.pt"):
            auc_std = [0.5] * 100
            ...
            cls_model = torch.load(f"Classifier_random\\remedis-random-{sample_id}-{bug}-{sub_bug}.pt")
            cls_model.eval()
...
results_df.to_csv(f"Classifier_performance\\remedis-random-{bug}.csv", index=False, encoding="cp1252")

...
reg_model.classifier[6] = nn.Linear(4096, 2048)
reg_model.to(device)
...
for epoch in range(epoch_num):
    ...
    if loss_tmp < loss_past:
        loss_past = loss_tmp
        PATH = f"Regressor_uncertainty\\VGG11-res152.pt"
...
mc_times = 100
output_path = "Monte_carlo\\res152"
summary_path = "Monte_carlo\\summary\\res152.csv"
reg_model = torch.load(f"Regressor_uncertainty\\VGG11-res152.pt")
...
uncertainty_csv = pd.read_csv("Monte_carlo\\summary\\res152.csv", header=None)
for sample_num in range(10, 51, 10):
    ...
    train_csv.iloc[np.concatenate([id_array])]. \
        to_csv(f"Sample_uncertainty\\train_res152_{sample_num}.csv", index=False)

...
for idx in range(10, 51, 10):
    ...
    for epoch in range(epoch_num):
        ...
        if loss_tmp < loss_past:
            loss_past = loss_tmp
            PATH = f"Classifier_uncertainty\\MLP3-uncertainty-remedis-{idx}.pt"
...
for idx in range(10, 51, 10):
    ...
    cls_model = torch.load(f"Classifier_uncertainty\\MLP3-uncertainty-remedis-{idx}.pt")
    cls_model.eval()
    prob_list = []
    label_list = []
    threshold = 0.5
...
results_df.to_csv("Classifier_performance\\MLP3-uncertainty-remedis.csv", index=False, encoding="cp1252")

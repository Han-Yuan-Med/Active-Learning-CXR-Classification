...
for sub_bug in range(10, 41, 10):
    ...
    if sub_bug == 10:
        train_csv_pre = pd.read_csv(f"Sample_hybrid\\train_txrv_10.csv").iloc[:bug, :]
        cls_model_pre = torch.load(f"Classifier_hybrid\\VGG11-hybrid-txrv-10.pt")
    else:
        train_csv_pre = pd.read_csv(f"Sample_hybrid\\train_txrv_{bug}_{sub_bug-10}.csv")
        cls_model_pre = torch.load(f"Classifier_hybrid\\VGG11-hybrid-txrv-{bug}-{sub_bug-10}.pt")
    ...
    train_csv_now.to_csv(f"Sample_hybrid\\train_txrv_{bug}_{sub_bug}.csv", index=False)
    ...
    for epoch in range(epoch_num):
        ...
        if loss_tmp < loss_past:
            loss_past = loss_tmp
            PATH = f"Classifier_hybrid\\VGG11-hybrid-txrv-{bug}-{sub_bug}.pt"
...
for sub_bug in range(10, 41, 10):
    ...
    cls_model = torch.load(f"Classifier_hybrid\\VGG11-hybrid-txrv-{bug}-{sub_bug}.pt")
    cls_model.eval()
    prob_list = []
    label_list = []
    threshold = 0.5
...
results_df.to_csv(f"Classifier_performance\\VGG11-hybrid-txrv-{bug}.csv", index=False, encoding="cp1252")

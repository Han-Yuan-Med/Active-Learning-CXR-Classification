...
for sub_bug in range(10, 41, 10):
    ...
    if sub_bug == 10:
        train_csv_pre = pd.read_csv(f"Sample_hybrid\\train_remedis_10.csv")
        cls_model_pre = torch.load(f"Classifier_hybrid\\MLP3-hybrid-remedis-10.pt")
    else:
        train_csv_pre = pd.read_csv(f"Sample_hybrid\\train_remedis_{bug}_{sub_bug-10}_MLP3.csv")
        cls_model_pre = torch.load(f"Classifier_hybrid\\MLP3-hybrid-remedis-{bug}-{sub_bug-10}.pt")
    ...
    train_csv_now.to_csv(f"Sample_hybrid\\train_remedis_{bug}_{sub_bug}_MLP3.csv", index=False)
    ...
    for epoch in range(epoch_num):
        ...
        if loss_tmp < loss_past:
            loss_past = loss_tmp
            PATH = f"Classifier_hybrid\\MLP3-hybrid-remedis-{bug}-{sub_bug}.pt"
...
for sub_bug in range(10, 41, 10):
    ...
    cls_model = torch.load(f"Classifier_hybrid\\MLP3-hybrid-remedis-{bug}-{sub_bug}.pt")
    cls_model.eval()
    prob_list = []
    label_list = []
    threshold = 0.5
...
results_df.to_csv(f"Classifier_performance\\MLP3-hybrid-remedis-{bug}.csv", index=False, encoding="cp1252")

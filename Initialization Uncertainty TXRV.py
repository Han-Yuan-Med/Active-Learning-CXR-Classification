...
for idx in range(10, 51, 10):
    ...
    for epoch in range(epoch_num):
        ...
        if loss_tmp < loss_past:
            loss_past = loss_tmp
            PATH = f"Classifier_uncertainty\\VGG11-uncertainty-txrv-{idx}.pt"
...
results_df = []
for idx in range(10, 51, 10):
    cls_model = torch.load(f"Classifier_uncertainty\\VGG11-uncertainty-txrv-{idx}.pt")
    cls_model.to(device)
    cls_model.eval()
    prob_list = []
    label_list = []
...
results_df.to_csv("Classifier_performance\\VGG11-uncertainty-txrv.csv", index=False, encoding="cp1252")

    ...
    for idx in range(10, 51, 10):
        ...
        for epoch in range(epoch_num):
            ...
            if loss_tmp < loss_past:
                loss_past = loss_tmp
                PATH = f"Classifier_random\\VGG11-random-{sample_id}-{idx}.pt"
...
for idx in range(10, 51, 10):
    ...
    bootstrap_npv = []
    for sample_id in range(100):
        if not os.path.exists(f"Classifier_random\\VGG11-random-{sample_id}-{idx}.pt"):
            auc_std = [0.5] * 100
            ...
            cls_model = torch.load(f"Classifier_random\\VGG11-random-{sample_id}-{idx}.pt")
            cls_model.eval()
...
results_df.to_csv("Classifier_performance\\VGG11-random.csv", index=False, encoding="cp1252")

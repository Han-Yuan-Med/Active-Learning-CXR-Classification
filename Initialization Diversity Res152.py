...
for idx in range(10, 51, 10):
    train_csv_tmp = pd.read_csv(f"Sample_diversity\\train_res152_{idx}.csv")
    ...
    for epoch in range(epoch_num):
        ...
        if loss_tmp < loss_past:
            loss_past = loss_tmp
            PATH = f"Classifier_diversity\\VGG11-diversity-res152-{idx}.pt"
...
for idx in range(10, 51, 10):
    ...
    cls_model = torch.load(f"Classifier_diversity\\VGG11-diversity-res152-{idx}.pt")
    cls_model.eval()
    prob_list = []
    label_list = []
    threshold = 0.5
...
results_df.to_csv("Classifier_performance\\VGG11-diversity-res152.csv", index=False, encoding="cp1252")
